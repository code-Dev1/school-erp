<div class="space-y-6">
    <section class="flex flex-col gap-4 rounded-2xl border border-white/70 bg-white/85 p-5 shadow-xl shadow-slate-950/[0.04] backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/70 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <h2 class="mt-1 text-2xl font-bold text-slate-950 dark:text-white">Create sale</h2>
        </div>
    ...
    <form wire:submit="save" class="space-y-6">
        <x-ui.card title="Sale information" icon="shopping-bag">
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <x-ui.input label="Invoice number" name="form.invoice_number" wire:model="form.invoice_number" />
                ...
                <x-ui.select label="Item" name="form.sale_item_id" :options="$itemOptions" placeholder="Select item" wire:model="form.sale_item_id" />
                <x-ui.input type="number" label="Quantity" name="form.quantity" min="1" wire:model="form.quantity" />
