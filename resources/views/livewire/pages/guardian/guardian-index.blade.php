<div class="space-y-6">
    <section class="flex flex-col gap-4 rounded-2xl border border-white/70 bg-white/85 p-5 shadow-xl shadow-slate-950/[0.04] backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/70 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <h2 class="mt-1 text-2xl font-bold tracking-normal text-slate-950 dark:text-white">سرپرستان</h2>
        </div>

        <x-ui.button href="{{ route('guardian-create') }}" icon="plus" wire:navigate>
            ثبت سرپرست
        </x-ui.button>
    </section>
    ...
    <x-ui.card>
        <div class="grid gap-4 md:grid-cols-[1fr_220px_auto] md:items-end">
            <x-ui.input
                ...
                label="جستجو"
                name="search"
                placeholder="نام، تماس، ایمیل یا تذکره"
                    ...
                    @forelse ($guardians as $guardian)
                        ...
                        <tr class="transition hover:bg-slate-50 dark:hover:bg-slate-900/80">
                            <td class="whitespace-nowrap px-4 py-4 font-medium text-slate-950 dark:text-white">
                                {{ $guardian->name ?: trim($guardian->first_name.' '.$guardian->last_name) }}
                            </td>
                            <td class="whitespace-nowrap px-4 py-4 text-slate-700 dark:text-slate-200">
                                {{ $guardian->phone ?: ($guardian->contact_number ?: 'ثبت نشده') }}
                            </td>
                            <td class="whitespace-nowrap px-4 py-4 text-slate-700 dark:text-slate-200">
                                {{ $guardian->job ?: ($guardian->occupation ?: 'ثبت نشده') }}
                            </td>
                            <td class="whitespace-nowrap px-4 py-4 text-slate-700 dark:text-slate-200">
                                {{ number_format($guardian->students_count) }}
                            </td>
                            <td class="whitespace-nowrap px-4 py-4">
                                <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ring-1 {{ $statusClass }}">
                                    {{ $statusOptions[$status] ?? 'ثبت نشده' }}
                                </span>
                            </td>
                            <td class="px-4 py-4">
                                <div class="flex justify-end">
                                    <x-ui.button
                                        ...
                                        wire:confirm="این سرپرست حذف شود؟"
                                    >
                                        حذف
                                    </x-ui.button>
                                </div>
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-14 text-center">
                                <div class="mx-auto flex max-w-sm flex-col items-center">
                                    ...
                                    <h3 class="mt-4 text-sm font-semibold text-slate-950 dark:text-white">سرپرستی ثبت نشده است</h3>
                                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">از دکمه ثبت سرپرست، اولین سرپرست را اضافه کنید.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
